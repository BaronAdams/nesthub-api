...
export const createManyPropertyValidator = [
  body('*.title')
    .isString()
  ...
  body('*.rooms')
    ...
    .custom((value, { req, path }) => {
      if (req.body[path.replace('rooms','property_type')] !== 'Terrain') {
        ...
        if (typeof bedrooms !== 'number') {
          throw new Error('Le nombre de chambres doit être un chiffre');
        }
        if (typeof livingRooms !== 'number') {
          throw new Error('Le nombre de salons doit être un chiffre');
        }
        if (typeof kitchens !== 'number') {
          throw new Error('Le nombre de cuisines doit être un chiffre');
        }
        if (typeof bathrooms !== 'number') {
          throw new Error('Le nombre de salles de bain doit être un chiffre');
        }
      }
      return true;
    }),
  ...
  body('*.rooms')
    ...
    .custom((rooms: any, { req, path }) => {
      const property_type  = req.body[path.replace('rooms','property_type')];
  ...
  body('*.furnitures')
    ...
    .custom((furnitures: any[], { req,path }) => {
      let furnished = req.body[path.replace('furnitures','furnished')];
      let property_type = req.body[path.replace('furnitures','property_type')]
